...
> **MCP 使用提示**：`tiangong_ai_remote` MCP 覆盖最权威的可持续性文献语料（约 7000 万 chunks、700 亿 token），检索时务必提供信息完备的查询上下文，以充分发挥混合检索效果；`tiangong_lca_remote` MCP 专注生命周期评估数据，需要细粒度 LCA 指标时与 `tiangong_ai_remote` 联合使用。
