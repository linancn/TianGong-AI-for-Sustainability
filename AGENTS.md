...
| **P5** | ACM Digital Library; Scopus; Web of Science; AntV MCP chart server; OpenAI Deep Research; Gemini Deep Research | Conditional | High-threshold or optional services used for expert synthesis and visualization once dependencies are met. Premium literature adapters now confirm API credential presence ahead of licensed ingestion. |
| **P_INT** | `tiangong_ai_remote` MCP; `dify_knowledge` REST API | Implemented | Project-specific RAG layer providing final contextualisation for automations. |
...
> **Retrieval Usage Notes** — The `tiangong_ai_remote` source remains the MCP automation endpoint, while `dify_knowledge` now calls the Dify Knowledge Base REST `POST /datasets/{dataset_id}/retrieve` endpoint with `Authorization: Bearer <API_KEY>` and a configured `dataset_id`; include full task intent and optional `retrieval_model` parameters (`search_method`, `top_k`, metadata filters) so the hybrid retriever returns high-signal passages. The `Search_*` tools (including `Search_Sci_Tool`) return a JSON string—decode it with `json.loads` before accessing fields and keep `topK` ≤ 50 to avoid oversized responses. Rate-limit follow-up enrichment calls (e.g., Semantic Scholar) or switch to `OpenAlex` when 429 throttling occurs. The `tiangong_lca_remote` MCP (P4) focuses on life-cycle assessment datasets; reserve it for micro-level LCA case studies or detailed footprint comparisons, and skip it for macro literature scans where P0/P2 sources suffice. Use `tavily_web_mcp` (P4) when you need general web or news coverage outside the curated TianGong corpus, remembering that the Tavily server expects `Authorization: Bearer <API_KEY>`. Treat `openai_deep_research` (P5) as an analysis source triggered after deterministic evidence collection; ensure the OpenAI API key and the deep research model are configured before enabling it in workflows. Gemini Deep Research uses the Google AI Interactions API; configure the [gemini] API key/agent before turning it on.
...
- Gemini Deep Research Interactions API now has a credential-aware adapter; synthesis remains optional and runs after deterministic evidence collection.
